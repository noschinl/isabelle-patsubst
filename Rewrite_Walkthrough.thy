...
apply(rewrite at "x + y" in "x + y + z" in concl all (x y z) add.commute)
apply(rewrite at "(y + _) + z" in concl all (y z) add.commute)
apply(rewrite at "_" in concl all () add.commute)
apply(simp)
done
...
apply(rewrite at "x + 1" in goal all (y x) at asm add.commute)
apply(simp)
done
