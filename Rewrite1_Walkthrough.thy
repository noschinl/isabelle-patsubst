theory Rewrite1_Walkthrough
imports Complex_Main Rewrite1 "~~/src/HOL/Library/While_Combinator"
begin
...
lemma
  ...
  apply(rewrite1 in "f _ + f \<box> = _" diff_self)
  apply(rewrite1 at "f (_ + \<box>) + f _ = _" diff_self)
  apply(rewrite1 in "f \<box> + _ = _" diff_self)

  (* Collapse zeros from left to right. *)
  apply(rewrite1 at "0 + 0" add.left_neutral)
  apply(rewrite1 at "0 + c" add.left_neutral)
...
apply (rewrite1 at "\<box> > c" add.commute)
apply assumption
done
...
apply(rewrite1 in asm add.commute)
apply(rewrite1 in "y + x > c" at asm add.commute)
apply(rewrite1 at "\<box> > c" at asm add.commute)
apply(assumption)
done
...
apply(rewrite1 at "x+1" in "{x::rat. \<box> }" add.commute)
oops

(* Slightly more complicated pattern. *)
lemma "P {(x::nat, y::nat, z). x + z * 3 = Q (\<lambda>s t. y + s * t - 3)}"
apply(rewrite1 at "b + d * e" in "\<lambda>(a, b, c). _ = Q (\<lambda>d e. \<box>)" add.commute)
oops
...
(*apply(rewrite1 test_theorem)*)
(* It makes sense to instantiate ?y beforehand. *)
apply(rewrite1 at "f x" test_theorem where y = 0)
oops
...
apply(rewrite1 at "3 + _" test_theorem where y=42)
oops
...
apply(rewrite1 f_inv_def[symmetric] where I = "\<lambda>x. True")
apply(rewrite1 f_inv_def)
...
apply(rewrite1 in "\<lambda>n. \<box>" f_inv_def[symmetric] where I = "\<lambda>x. n < x + 1")
apply(rewrite1 f_inv_def)

(* Any identifier will work *)
apply(rewrite1 in "\<lambda>abc. \<box>" f_inv_def[symmetric] where I = "\<lambda>x. abc < x + 1")
apply(rewrite1 f_inv_def)
oops

(* The "all" keyword. *)
lemma "\<And>x y z. x + y + z = z + y + (x::int)"
apply(rewrite1 at "x + y" in "x + y + z" in concl all (x y z) add.commute)
apply(rewrite1 at "(y + _) + z" in concl all (y z) add.commute)
apply(rewrite1 at "_" in concl all () add.commute)
apply(simp)
done
...
apply(rewrite1 at "x + 1" in goal all (x) at asm add.commute)
apply(simp)
done
...
apply(rewrite1 in "snd (while _ (\<lambda>(i, _). \<box>) _)" while_inv_def[symmetric] where ?I = "\<lambda>(j::nat, x). x = j + 3*i" )
oops
