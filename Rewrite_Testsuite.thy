...
by (rewrite in "P (1 + 2)" at "_" all (x) add.commute)
   (rule assms)

lemma
  assumes "\<And>x y. P (y + x)"
  shows "\<And>x y. P (x + y :: nat)"
by (rewrite in "P (x + _)" at "_" all (x y) add.commute)
   (rule assms)

lemma
  assumes "\<And>x y z. y + x + z = z + y + (x::int)"
  shows   "\<And>x y z. x + y + z = z + y + (x::int)"
by (rewrite at "x + y" in "x + y + z" in concl all (x y z) add.commute)
   (rule assms)
   
lemma
  assumes "\<And>x y z. z + (x + y) = z + y + (x::int)"
  shows   "\<And>x y z. x + y + z = z + y + (x::int)"
by (rewrite at "(_ + y) + z" in concl all (y z) add.commute)
   (rule assms)
   
lemma
  assumes "\<And>x y z. x + y + z = y + z + (x::int)"
  shows   "\<And>x y z. x + y + z = z + y + (x::int)"
by (rewrite at "\<box> + _" at "_ = \<box>" in concl all () add.commute)
   (rule assms)
...
by (rewrite at "x + 1" in goal all (x) at asm add.commute)
   (rule assms)
